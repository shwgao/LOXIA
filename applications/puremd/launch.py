    ...
    for i, (data) in enumerate(train_loader):
        ...
        if i % 100 == 0:
            print(' Epoch: [{0}][{1}/{2}]\t'
                  ...
                  'R2 {acc.val:.6f} ({acc.avg:.6f})'.format(epoch, i, len(train_loader),
                                                            loss=losses, l0_loss=l0_losses, acc=r2))
...
def validate(val_loader, model, criterion, inference=False):
    ...
    print('Test: [{0}]\t Loss {loss.val:.6f} ({loss.avg:.6f})\t R2 {r2.val:.6f} ({r2.avg:.6f})'.format(len(val_loader),
                                                                                                       loss=losses,
                                                                                                       r2=r2))
